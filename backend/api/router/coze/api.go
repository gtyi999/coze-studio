...
func Register(r *server.Hertz) {
	...
	root := r.Group("/", rootMw()...)
	{
		...
		{
			_im := _api.Group("/im", _imMw()...)
			_im.GET("/platforms", append(_listimplatformsMw(), coze.ListIMPlatforms)...)
			{
				_dingtalk := _im.Group("/dingtalk", _dingtalkMw()...)
				_dingtalk.GET("/event", append(_dingtalkeventMw(), coze.DingTalkEvent)...)
				_dingtalk.POST("/event", append(_dingtalkevent0Mw(), coze.DingTalkEvent)...)
			}
			{
				_feishu := _im.Group("/feishu", _feishuMw()...)
				_feishu.POST("/event", append(_feishueventMw(), coze.FeishuEvent)...)
			}
			{
				_wecom := _im.Group("/wecom", _wecomMw()...)
				_wecom.GET("/event", append(_wecomeventMw(), coze.WeComEvent)...)
				_wecom.POST("/event", append(_wecomevent0Mw(), coze.WeComEvent)...)
			}
		}
		{
